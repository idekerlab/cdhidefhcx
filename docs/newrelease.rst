...
This is an advanced developer section that describes creation of a new release for cdhidefhcx and deploying
...
Be sure to update the version in ``cdhidefhcx/__init__.py`` Omit the ``v`` when setting the version in the file.

Commit the above changes and push to **main**:

.. code-block::

    git commit -m 'message...' cdhidefhcx/__init__.py
...
From cdhidefhcx source tree generate the wheel and tar.gz files
...
.. code-block::
    ...
    $ tree
    dist
    |-- cdhidefhcx-0.1.0-py2.py3-none-any.whl
    `-- cdhidefhcx-0.1.0.tar.gz
...
#. From https://github.com/idekerlab/cdhidefhcx click on releases link.
...
Once the above is done verify deploy was successful by browsing to https://testpypi.python.org/pypi/cdhidefhcx and verifying new release was deployed.

Also test the package by installing it locally by running this:

.. code-block::

    pip install -i https://testpypi.python.org/pypi cdhidefhcx
...
.. code-block::
    ...
    pip install cdhidefhcx


.. _Travis: https://travis-ci.org/idekerlab/cdhidefhcx
.. _main: https://github.com/idekerlab/cdhidefhcx/tree/main
